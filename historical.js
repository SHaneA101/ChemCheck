...
async function renderReports() {
  if (shouldUseLocalStorageOnly()) {
    renderReportMarkup(getStorageBridge()?.getReports() || []);
    return;
  }

  try {
    const reports = await requestJson(REPORTS_ENDPOINT);
    renderReportMarkup(reports);
  } catch (error) {
    const localReports = getStorageBridge()?.getReports();
    if (localReports?.length) {
      renderReportMarkup(localReports);
      return;
    }
...
async function renderHistory() {
  if (shouldUseLocalStorageOnly()) {
    renderHistoryMarkup(getStorageBridge()?.getHistory() || []);
    return;
  }

  try {
    const entries = await requestJson(HISTORY_ENDPOINT);
    renderHistoryMarkup(entries);
  } catch (error) {
    const localEntries = getStorageBridge()?.getHistory();
    if (localEntries?.length) {
      renderHistoryMarkup(localEntries);
      return;
    }
...
async function clearHistory() {
  ...
  if (shouldUseLocalStorageOnly()) {
    storageBridge?.clearHistory();
    storageBridge?.clearReports();
    await initialize();
...
function renderReportMarkup(reports) {
  if (!reports.length) {
    reportList.innerHTML = `
      <div class="history-empty">
...
function renderHistoryMarkup(entries) {
  if (!entries.length) {
    historyList.innerHTML = `
      <div class="history-empty">
